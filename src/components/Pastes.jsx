...
const Pastes = () => {
  ...
  // fiter pastes based on search term (by title or content)
      ...
      <div className="flex flex-col gap-y-3">
       ...
       filteredData.length>0 ?
        filteredData.map(
          (paste)=> {
            return (
              <div className="border border-[rgba(128,121,121,0.3)] w-full gap-y-6 justify-between flex flex-col sm:flex-row p-4 rounded-[0.3rem]">

                <div className="w-[50%] flex flex-col space-y-3" >
                  {paste.title}
                </div>
                <div>
                 <p  className="text-4xl font-semibold "> {paste.content}</p>
                ...
                {/* icons */}
                 <div  className="flex flex-col gap-y-4 sm:items-end">
                  ...
                  {/* edit */}
                  <button  className="p-2 rounded-[0.2rem] bg-white border border-[#c7c7c7] 
                   hover:bg-transparent group hover:border-blue-500">
                   <a href={`/?pasteId=${paste.id}`}>
                    <PencilLine
                     ...
                     {/* view icon */}
                  <button className="p-2 rounded-[0.2rem] bg-white border
                   border-[#c7c7c7]  hover:bg-transparent group hover:border-orange-500">
                  ...
                  <button onClick={()=>handleDelete(paste.id)}
                     className="p-2 rounded-[0.2rem] bg-white border border-[#c7c7c7] 
                      hover:bg-transparent group hover:border-pink-500"
                    >
                      <Trash2
